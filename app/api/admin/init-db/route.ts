...
CREATE INDEX IF NOT EXISTS idx_evaluation_runs_timestamp ON public.evaluation_runs(run_timestamp);
CREATE INDEX IF NOT EXISTS idx_conversation_scores_eval_run ON public.conversation_scores(evaluation_run_id);
CREATE INDEX IF NOT EXISTS idx_conversation_scores_case ON public.conversation_scores(case_id);
CREATE INDEX IF NOT EXISTS idx_prompt_variants_agent_adopted ON public.prompt_variants(agent_name, adopted_at);
CREATE INDEX IF NOT EXISTS idx_cost_log_component ON public.cost_log(component);
CREATE INDEX IF NOT EXISTS idx_temporal_events_case ON public.temporal_events(case_id);
`;
...
export async function POST() {
  ...
  try {
    ...
    // Use fetch to execute SQL via Supabase API
    const response = await fetch(`${supabaseUrl}/rest/v1/rpc/exec_sql`, {
      ...
      body: JSON.stringify({ sql_text: schemaSQL }),
    });

    if (!response.ok) {
      const errorText = await response.text();
      console.error('[v0] RPC Error:', errorText);
      return NextResponse.json(
        { 
