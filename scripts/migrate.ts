...
async function runMigration() {
  console.log("[v0] Starting database schema initialization...");
  ...
  for (const statement of statements) {
    try {
      ...
      if (error) {
        // Some errors are expected (table already exists), so we continue
        if (error.message?.includes("already exists")) {
          console.log("[v0] Table already exists (skipping):", statement.substring(0, 50));
        } else {
  ...
  console.log(`[v0] Migration complete. Success: ${successCount}, Errors: ${errorCount}`);
}
